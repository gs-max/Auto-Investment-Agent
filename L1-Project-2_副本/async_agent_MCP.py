    ...
    def _build_routing_config(self, tools):
        ...
        for tool in tools:
            ...
            # 如果工具名称不包含 "retrieve"
                ...
                # 记录调试日志，说明该工具被路由到 "agent"
                logger.debug(f"Tool '{tool_name}' routed to 'agent'")
            else:
...
# 定义 Node agent分诊函数
async def agent(state: MessagesState, config: RunnableConfig, *, store: BaseStore, llm_chat, tool_config: ToolConfig) -> dict:
    ...
    try:
        ...
        # 将工具绑定到 LLM
        logger.info("---------------------------------------------------------------------")
        logger.info(f"tool_config.get_tools():{tool_config.get_tools()}")
        logger.info("---------------------------------------------------------------------")
        ...
        # serialized_response = {"content": response, "type": "ai"}
...
def generate(state: MessagesState, llm_chat) -> dict:
    """
    基于工具返回的内容生成最终回复。
    这个节点作为流程的最后一站，只返回最终的答案消息。
    """
    logger.info("Generating final response")
    
    try:
        # --- 步骤 1: 提取必要的上下文 ---
        # 我们只需要最近的用户问题和最近的工具输出
        ...
        # 找到最近的用户问题
        question = ""
        # 筛选掉 ToolMessage 来找用户问题
        history_without_tools = [msg for msg in state["messages"] if not isinstance(msg, ToolMessage)]
        if history_without_tools:
            question = get_latest_question({"messages": history_without_tools})
        ...
        response = generate_chain.invoke({"context": last_tool_output, "question": question})
        print('last_tool_output:',last_tool_output)
        print('question:',question)
        # --- 步骤 3: 只返回本次生成的最终答案 ---
        # *** 这是最关键的修改 ***
        # 我们返回一个只包含最终 AIMessage 的新列表。
        # LangGraph 的 add_messages 会用这个新列表覆盖掉整个旧的 messages 状态。
        # 这样，当 checkpointer 保存时，它只需要保存这一个最终消息，
        # 而不是整个冗长的历史。
        print("final_messages:", [response]) # 打印一下确认
        return {"messages": [response]}

    except Exception as e:
        logger.error(f"Error in generate: {e}", exc_info=True)
        return {"messages": [SystemMessage(content="生成最终回复时发生未知错误")]}
...
def create_graph(db_connection_pool: ConnectionPool, llm_chat, llm_embedding, tool_config: ToolConfig) -> StateGraph:
    ...
    workflow.add_node("agent", agent_with_args)
    # 添加工具节点，使用并行工具节点
    ...
    workflow.add_conditional_edges(source="call_tools", path=lambda state: route_after_tools(state, tool_config),path_map={"generate": "generate", "grade_documents": "grade_documents", "agent": "agent"})
...
# 定义响应函数
async def graph_response(graph: StateGraph, user_input: str, config: dict, tool_config: ToolConfig) -> None:
    ...
    try:
        ...
        events = graph.astream({"messages": [{"role": "user", "content": user_input}], "rewrite_count": 0}, config)
        ...
        async for event in events:
            ...
            for value in event.values():
                ...
                # print(last_message)
                print('last_message_type',type(last_message))
    ...
    try:
        ...
        while True:
            ...
            if not user_input:
                ...
                continue
            # 处理用户输入并选择是否流式输出响应
            await graph_response(graph, user_input, config, tool_config)
